...
from ramka import Game, Sprite, Vector, Camera, Animation
...
class Oblachko(Sprite):
    ...
    def __init__(self, dir):
        if not Oblachko.anim:
            a1=Animation("img/Particles/oblachko1.png", 5, True)
            a2=Animation("img/Particles/oblachko2.png", 5, True)
            Oblachko.anim=[a1,a1,a1,a2]

        super().__init__(choice(Oblachko.anim))
    ...
    def update(self, deltaTime: float):
        ...
        aspd= deltaTime * spd_interp(self.time / self.life_time) * self.lspd
        step = self.spd * aspd

        self.transform.pos += step
        self.transform.angle+=max(6*aspd,50*deltaTime)
