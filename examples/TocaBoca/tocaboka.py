...
# todo: предмет нельзя прикрепить к DropZone если 1) он шире чем докер 2) если место занято 3) если не подходящий класс, 3) нельзя прикрепить если сумка закрыта (от состояния)
