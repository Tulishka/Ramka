...
def main():
    ...
    for i, p1 in enumerate(box):
        if i + 1 >= len(box):
            ...
            p2 = box[i + 1]
        l = pymunk.Segment(space1.static_body, p1, p2, 10)
        l.elasticity = 0.5
        l.friction = 1
    ...
    for x in range(3):
        for y in range(7):
            ...
            box.body.position = 200 + x * 30, 290 - y * 40
            space1.add(box, box.body)

    b = pymunk.Body()
    b.position = 30, 270
    ball = pymunk.Circle(b, 20,(0,20))
    ball.mass = 20
    ball.friction = 1
    ball.color = pygame.Color("red")

    ball2 = pymunk.Circle(b, 20,(0,-20))
    ball2.mass = 20
    ball2.friction = 1
    ball2.color = pygame.Color("green")

    space1.add(b,ball,ball2)
